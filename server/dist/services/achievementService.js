...
exports.checkAchievementsForUser = checkAchievementsForUser;
...
async function checkAndAwardAchievements(userId, context) {
    ...
    for (const check of checks) {
        ...
        if (!def)
            continue;
        const exists = await prisma_1.prisma.achievement.findFirst({
...
async function checkAchievementsForUser(userId) {
    const [tasksCompleted, pomodoroStats, habits, friendships, user, goalsCompleted, totalGoldEarned,] = await Promise.all([
        ...
        prisma_1.prisma.habit.findMany({ where: { userId }, select: { currentStreak: true, bestStreak: true } }),
        ...
        prisma_1.prisma.user.findUnique({ where: { id: userId }, select: { gold: true, level: true } }),
        prisma_1.prisma.goal.count({ where: { userId, status: 'completed' } }),
        // Суммарное заработанное золото из транзакций (не текущий баланс)
        prisma_1.prisma.rewardTransaction.aggregate({
            where: {
                userId,
                rewardType: 'gold',
                amount: {
