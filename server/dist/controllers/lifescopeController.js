...
exports.getArchive = exports.getCurrentReport = void 0;
...
async function buildWeeklyReport(userId, weekStart, weekEnd) {
    const [sessions, tasks, habits, rewards, prevRewards] = await Promise.all([
        ...
        prisma_1.prisma.rewardTransaction.findMany({
            where: {
                userId,
                createdAt: { gte: (0, date_fns_1.subWeeks)(weekStart, 1), lte: (0, date_fns_1.subWeeks)(weekEnd, 1) }
            },
            select: { rewardType: true, amount: true }
        }),
    ]);
    // Помодоро по категориям
    const pomodoroByCategory = {};
    ...
    return {
        ...
        weekLabel: `${(0, date_fns_1.format)(weekStart, 'd MMM', { locale: locale_1.ru })} – ${(0, date_fns_1.format)(weekEnd, 'd MMM yyyy', { locale: locale_1.ru })}`,
        totalPomodoroMin,
        pomodoroByCategory,
        tasksCompleted: tasks.length,
...
const getCurrentReport = async (req, res) => {
    try {
        const now = new Date();
...
const getArchive = async (req, res) => {
    try {
        ...
        for (let i = 1; i <= Math.min(weeks, 12); i++) {
            ...
            const weekEnd = (0, date_fns_1.endOfWeek)((0, date_fns_1.subWeeks)(now, i), { weekStartsOn: 1 });
            const report = await buildWeeklyReport(req.userId, weekStart, weekEnd);
            reports.push(report);
        }
        res.json({ reports });
